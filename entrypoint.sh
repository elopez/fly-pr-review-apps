...
remote_build=
if [ "$INPUT_REMOTE" != "false" ]; then
  remote_build="--remote-only"
fi
...
if ! flyctl status --app "$app"; then
  ...
  flyctl deploy --app "$app" --region "$region" --image "$image" --region "$region" --strategy immediate $remote_build
elif [ "$INPUT_UPDATE" != "false" ]; then
  flyctl deploy --config "$config" --app "$app" --region "$region" --image "$image" --region "$region" --strategy immediate $remote_build
fi
